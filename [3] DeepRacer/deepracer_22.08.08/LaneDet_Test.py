...
def RANSAC(x, y, n) :
    ...
    for itr in range(N) :
        k = np.floor(n_data*np.random.rand(n_sample)); k = k.astype(int)
        if n == 2:
...
def frame_test(frame, cnt):
    ...
    plt.grid(True)
    plt.axis("equal")
    img_mask_L = LaneDet(img_rgb, lower_red, upper_red, crop_lineX_lower, crop_lineX_upper, crop_curvX_lower, crop_curvX_upper, [1, 0, 0])
    img_mask_R = LaneDet(img_rgb, lower_green, upper_green, crop_lineX_lower, crop_lineX_upper, crop_curvX_lower, crop_curvX_upper, [0, 1, 0])
    ...
    print(f" Experiment ID : ", cnt)
    plt.savefig(f"exp_" + str(cnt) + ".png")
    plt.close()

#frame = cv.imread("/home/vdcl/Desktop/DeepRacer_ver2/test1659773140.1157727.png")
#frame_test(frame,1)
