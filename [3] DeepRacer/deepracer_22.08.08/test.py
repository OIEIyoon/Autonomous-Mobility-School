...
def detect_green(frame):
    ...
    upper_red = np.array([50, 255, 255], dtype="uint8")
    mask_red = cv2.inRange(rgb, lower_red, upper_red)
    # mask_white = cv2.inRange(gray, 200, 255)
    # mask_bu = cv2.bitwise_or(mask_white, mask_blue)
    # mask_bu = cv2.bitwise_or(mask_blue)
...
def region_of_interest_first(edges):
    ...
    polygon = np.array([[0, h - 1/5*h], [0, h - 1/2*h], [1/20 * w, h - 2/3 * h], [19/20*w, h - 2/3*h],  
    [w, h - 1/2 * h], [w, h - 1/5 * h]], np.int32) 
    
    #polygon = np.array([[0, h], [w, h], [w, 0], [0, 0]], np.int32)  
...
def region_of_interest_second(edges):
    ...
    # only focus bottom half of the screen
    polygon = np.array([
    [0, h - 1/5*h], [0, h - 1/2*h], [1/20 * w, h - 2/3 * h], [19/20*w, h - 2/3*h],  
    [w, h - 1/2 * h], [w, h - 1/5 * h]], np.int32)
