...
class Camera:
    def __init__(self, path = "/dev", crop_lineX_lower = 0.3, crop_lineX_upper = 0.6, crop_curvX_lower = 0.5, crop_curvX_upper = 1):
        self.v_num = 0
        self.VIDEO_PATH = path + "/video" + str(self.v_num)
        ...
        # RGB range
        self.lower_red = np.array([245, 100, 110], dtype="uint8")
        self.upper_red = np.array([255, 180, 255], dtype="uint8")

        self.lower_green = np.array([0, 200, 180], dtype="uint8")
        self.upper_green = np.array([100, 255, 255], dtype="uint8")

        self.lower_white = np.array([240, 240, 240], dtype="uint8")
        self.upper_white = np.array([255, 255, 255], dtype="uint8")

        self.lower_blue = np.array([0, 140, 220], dtype="uint8")
        self.upper_blue = np.array([100, 200, 255], dtype="uint8")
    ...
    def imwrite(self):
        try:
            cv.imwrite('test' + str(self.c_cnt) + '.png', self.frame)
            self.c_cnt += 1
        except:
